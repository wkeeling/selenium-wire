...
import seleniumwire.proxy.types
from seleniumwire.proxy import command_lexer, exceptions
from seleniumwire.proxy.command_lexer import unquote
...
def typename(t: type) -> str:
    ...
    to = seleniumwire.proxy.types.CommandTypes.get(t, None)
    if not to:
...
class Command:
    ...
    def __init__(self, manager: "CommandManager", name: str, func: typing.Callable) -> None:
        ...
        for name, parameter in self.signature.parameters.items():
            ...
            if not seleniumwire.proxy.types.CommandTypes.get(parameter.annotation, None):
                raise exceptions.CommandError(f"Argument {name} has an unknown type ({_empty_as_none(t)}) in {func}.")
        if self.return_type and not seleniumwire.proxy.types.CommandTypes.get(self.return_type, None):
    ...
    def call(self, args: typing.Sequence[str]) -> typing.Any:
        ...
        if ret is None and self.return_type is None:
            return
        typ = seleniumwire.proxy.types.CommandTypes.get(self.return_type)
        assert typ
...
class CommandManager:
    ...
    def parse_partial(
        ...
        next_params: typing.List[CommandParameter] = [
            CommandParameter("", seleniumwire.proxy.types.Cmd),
            CommandParameter("", seleniumwire.proxy.types.CmdArgs),
        ]
        ...
        for part in parts:
            if part.isspace():
                parsed.append(
                    ParseResult(
                        value=part,
                        type=seleniumwire.proxy.types.Space,
                        valid=True,
                    )
                )
                continue
            ...
            if expected and expected.kind is inspect.Parameter.VAR_POSITIONAL:
                ...
                expected = CommandParameter("", seleniumwire.proxy.types.Unknown)

            arg_is_known_command = (
                    expected.type == seleniumwire.proxy.types.Cmd and part in self.commands
            )
            arg_is_unknown_command = (
                    expected.type == seleniumwire.proxy.types.Cmd and part not in self.commands
            )
            command_args_following = (
                    next_params and next_params[0].type == seleniumwire.proxy.types.CmdArgs
            )
            ...
            to = seleniumwire.proxy.types.CommandTypes.get(expected.type, None)
            valid = False
            if to:
                try:
    ...
    def execute(self, cmdstr: str) -> typing.Any:
        ...
        command_name, *args = [
            unquote(part.value)
            for part in parts
            if part.type != seleniumwire.proxy.types.Space
        ]
...
def parsearg(manager: CommandManager, spec: str, argtype: type) -> typing.Any:
    ...
    """
    t = seleniumwire.proxy.types.CommandTypes.get(argtype, None)
    if not t:
...
def argument(name, type):
    """
        ...
        specific types such as seleniumwire.proxy.types.Choice, which we cannot annotate
