...
import seleniumwire.proxy.net.http
from seleniumwire.proxy import connections, exceptions, flow, http  # noqa
from seleniumwire.proxy.coretypes import basethread
from seleniumwire.proxy.net import tcp
from seleniumwire.proxy.net.http import headers, http2, url
from seleniumwire.proxy.server.protocol import base
from seleniumwire.proxy.server.protocol import http as httpbase
from seleniumwire.proxy.utils import human
...
class Http2Layer(base.Layer):
    ...
    def _handle_request_received(self, eid, event):
        headers = seleniumwire.proxy.net.http.Headers([[k, v] for k, v in event.headers])
    ...
    def _handle_trailers(self, eid, event, is_server, other_conn):
        trailers = seleniumwire.proxy.net.http.Headers([[k, v] for k, v in event.headers])
...
class Http2SingleStreamLayer(httpbase._HttpTransmissionLayer, basethread.BaseThread):
    ...
    class Message:
        def __init__(self, headers=None):
            self.headers: Optional[seleniumwire.proxy.net.http.Headers] = headers  # headers are the first thing to be received on a new stream
            self.data_queue: queue.Queue[bytes] = queue.Queue()  # contains raw contents of DATA frames
            self.queued_data_length = 0  # used to enforce seleniumwire.proxy's config.options.body_size_limit
            self.trailers: Optional[seleniumwire.proxy.net.http.Headers] = None  # trailers are received after stream_ended is set
    ...
    def __init__(self, ctx, h2_connection, stream_id: int, request_headers: seleniumwire.proxy.net.http.Headers) -> None:
        super().__init__(
