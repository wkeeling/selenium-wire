...
import seleniumwire.proxy.net.http.url
from seleniumwire.proxy.coretypes import multidict
from seleniumwire.proxy.net.http import cookies, message, multipart
from seleniumwire.proxy.net.http.headers import Headers
from seleniumwire.proxy.utils.strutils import always_bytes, always_str
...
class Request(message.Message):
    ...
    def host(self, val: Union[str, bytes]) -> None:
        ...
        if self.data.authority:
            self.authority = seleniumwire.proxy.net.http.url.hostport(self.scheme, self.host, self.port)
    ...
    def url(self) -> str:
        ...
        return seleniumwire.proxy.net.http.url.unparse(self.scheme, self.host, self.port, self.path)

    @url.setter
    def url(self, val: Union[str, bytes]) -> None:
        val = always_str(val, "utf-8", "surrogateescape")
        self.scheme, self.host, self.port, self.path = seleniumwire.proxy.net.http.url.parse(val)
    ...
    def pretty_host(self) -> str:
        ...
        if authority:
            return seleniumwire.proxy.net.http.url.parse_authority(authority, check=False)[0]
        else:
            return self.host
    ...
    def pretty_url(self) -> str:
        ...
        pretty_host, pretty_port = seleniumwire.proxy.net.http.url.parse_authority(host_header, check=False)
        pretty_port = pretty_port or seleniumwire.proxy.net.http.url.default_port(self.scheme) or 443

        return seleniumwire.proxy.net.http.url.unparse(self.scheme, pretty_host, pretty_port, self.path)

    def _get_query(self):
        query = urllib.parse.urlparse(self.url).query
        return tuple(seleniumwire.proxy.net.http.url.decode(query))

    def _set_query(self, query_data):
        query = seleniumwire.proxy.net.http.url.encode(query_data)
    ...
    def query(self) -> multidict.MultiDictView:
        """
        The request query string as an :py:class:`~seleniumwire.proxy.net.multidict.MultiDictView` object.
        """
    ...
    def cookies(self) -> multidict.MultiDictView:
        ...
        An empty :py:class:`~seleniumwire.proxy.net.multidict.MultiDictView` object if the cookie monster ate them all.
        """
        return multidict.MultiDictView(
            self._get_cookies,
            self._set_cookies
    ...
    def path_components(self):
        ...
        return tuple(seleniumwire.proxy.net.http.url.unquote(i) for i in path.split("/") if i)

    @path_components.setter
    def path_components(self, components):
        components = map(lambda x: seleniumwire.proxy.net.http.url.quote(x, safe=""), components)
    ...
    def _get_urlencoded_form(self):
        ...
        if is_valid_content_type:
            return tuple(seleniumwire.proxy.net.http.url.decode(self.get_text(strict=False)))
        return ()
    ...
    def _set_urlencoded_form(self, form_data):
        ...
        self.content = seleniumwire.proxy.net.http.url.encode(form_data, self.get_text(strict=False)).encode()

    @property
    def urlencoded_form(self):
        """
        The URL-encoded form data as an :py:class:`~seleniumwire.proxy.net.multidict.MultiDictView` object.
    ...
    def _set_multipart_form(self, value):
        self.content = seleniumwire.proxy.net.http.multipart.encode(self.headers, value)
        self.headers["content-type"] = "multipart/form-data"

    @property
    def multipart_form(self):
        """
        The multipart form data as an :py:class:`~seleniumwire.proxy.net.multidict.MultiDictView` object.
        An empty multidict.MultiDictView if the content-type indicates non-form data
        or the content could not be parsed.
