...
def get_default_proxy():
    """Returns the default mitmproxy, set by set_default_proxy."""
...
def wrap_module(module):
    ...
    Must set a default mitmproxy using set_default_proxy(...) first. This will
    only work on modules that import socket directly into the namespace;
    most of the Python Standard Library falls into this category."""
    if socksocket.default_proxy:
        module.socket.socket = socksocket
    else:
        raise GeneralProxyError("No default mitmproxy specified")
...
def create_connection(dest_pair,
    ...
    Like socket.create_connection(), but connects to mitmproxy
    ...
    **proxy_args - Same args passed to socksocket.set_proxy() if present.
    ...
    """
    # Remove IPv6 brackets on the remote address and mitmproxy address.
    ...
    # Allow the SOCKS mitmproxy to be on IPv4 or IPv6 addresses.
...
class socksocket(_BaseSocket):
    ...
    def set_proxy(self, proxy_type=None, addr=None, port=None, rdns=True,
                  username=None, password=None):
        """ Sets the mitmproxy to be used.

        proxy_type -  The type of the mitmproxy to be used. Three types
                        are supported: PROXY_TYPE_SOCKS4 (including socks4a),
                        PROXY_TYPE_SOCKS5 and PROXY_TYPE_HTTP
        addr -        The address of the server (IP or DNS).
        port -        The port of the server. Defaults to 1080 for SOCKS
                        servers and 8080 for HTTP mitmproxy servers.
    ...
    def bind(self, *pos, **kw):
        """Implements mitmproxy connection for UDP sockets.
        ...
        if proxy_type != SOCKS5:
            msg = "UDP only supported by SOCKS5 mitmproxy type"
        ...
        # The relay is most likely on the same host as the SOCKS mitmproxy,
    ...
    def get_proxy_sockname(self):
        """Returns the bound IP address and port number at the mitmproxy."""
    ...
    def get_proxy_peername(self):
        """
        Returns the IP and port number of the mitmproxy.
        """
    ...
    def get_peername(self):
        ...
        Note: get_proxy_peername returns the mitmproxy."""
        ...
        try:
            ...
            if chosen_auth[0:1] != b"\x05":
                ...
                raise GeneralProxyError(
                    "SOCKS5 mitmproxy server sent invalid data")
    ...
    def _read_SOCKS5_address(self, file):
        ...
        if atyp == b"\x01":
            ...
            raise GeneralProxyError("SOCKS5 mitmproxy server sent invalid data")
    ...
    def _negotiate_SOCKS4(self, dest_addr, dest_port):
        ...
        try:
            ...
            if resp[0:1] != b"\x00":
                # Bad data
                raise GeneralProxyError(
                    "SOCKS4 mitmproxy server sent invalid data")
    ...
    def _negotiate_HTTP(self, dest_addr, dest_port):
        ...
        try:
            ...
            raise GeneralProxyError("HTTP mitmproxy server sent invalid response")

        if not proto.startswith("HTTP/"):
            raise GeneralProxyError(
                "Proxy server does not appear to be an HTTP mitmproxy")

        try:
            status_code = int(status_code)
        except ValueError:
            raise HTTPError(
                "HTTP mitmproxy server did not return a valid HTTP status")

        if status_code != 200:
            error = "{}: {}".format(status_code, status_msg)
            if status_code in (400, 403, 405):
                # It's likely that the HTTP mitmproxy server does not support the
                # CONNECT tunneling method
                error += ("\n[*] Note: The HTTP mitmproxy server may not be"
    ...
    def connect(self, dest_pair, catch_errors=None):
        """
        Connects to the specified destination through a mitmproxy.
        Uses the same API as socket's connect().
        To select the mitmproxy server, use set_proxy().
        ...
        try:
            # Initial connection to mitmproxy server.
            super(socksocket, self).connect(proxy_addr)

        except socket.error as error:
            # Error while connecting to mitmproxy
            self.close()
            if not catch_errors:
                ...
                msg = "Error connecting to {} mitmproxy {}".format(printable_type,
                                                               proxy_server)
            ...
            # Connected to mitmproxy server, now negotiate
            try:
                ...
                # Protocol error while negotiating with mitmproxy
                self.close()
                raise
    ...
    def _proxy_addr(self):
        """
        Return mitmproxy address to connect to as tuple object
        """
        (proxy_type, proxy_addr, proxy_port, rdns, username,
         password) = self.proxy
        proxy_port = proxy_port or DEFAULT_PORTS.get(proxy_type)
        if not proxy_port:
            raise GeneralProxyError("Invalid mitmproxy type")
